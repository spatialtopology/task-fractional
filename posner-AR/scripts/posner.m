...
counterbalancefile             = fullfile(main_dir,'design', 's02_counterbalance', [input_counterbalance_file, '.csv']);
...
instruct_start                 = 'The mental rotation task is about to start. \n Please wait for the experimenter';
...
if string(countBalMat.cue{trl}) == 'left';
    ...
    T.p2_cue_duration(trl) = cue_offset-cue_onset;
...
if string(countBalMat.target{trl}) == 'left';
    ...
    % Screen('FrameRect', p.ptb.window, p.ptb.green, p.rect.leftRects, p.rect.penWidthPixels);
    T.target_onset(trl) = Screen('Flip', p.ptb.window);
%     WaitSecs(0.200);
